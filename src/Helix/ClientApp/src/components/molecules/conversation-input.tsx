...
export default function ConversationInput({ onSubmitPrompt }: ConversationInputProps) {
    const form = useForm({
            ...
            <Form {...form}>
                <FormField control={form.control} name="prompt" render={({ field }) => (
                    <FormItem>
                        ...
                        <FormMessage />
                        <FormDescription>
                            Press <kbd className="rounded border bg-muted px-1 text-xs font-semibold">{modifierKey}</kbd> + <kbd className="rounded border bg-muted px-1 text-xs font-semibold">Enter</kbd> to submit.
                        </FormDescription>
                    </FormItem>
                )} />
            </Form>
